...
static void _finishTransfer(struct GBSIOLockstepNode* node) {
	...
	uint8_t incoming = node->p->pendingSB[!node->id];
	struct GBSIO* sio = node->d.p;

	// === INCOMING DATA (READ) ===
	gb_link_cable_on_read(node->id, incoming, 8);

	sio->pendingSB = incoming;
...
static int32_t _masterUpdate(struct GBSIOLockstepNode* node) {
	...
	case TRANSFER_FINISHING:
		node->nextEvent += node->d.p->period * (2 - node->d.p->p->doubleSpeed) - 8;
#ifndef NDEBUG
...
static void _GBSIOLockstepNodeProcessEvents(struct mTiming* timing, void* user, uint32_t cyclesLate) {
	...
	if (node->p->d.attached < 2) {
		mTimingSchedule(
			timing,
			&node->event,
			(GBSIOCyclesPerTransfer[0] >> 1) * (2 - node->d.p->p->doubleSpeed) - cyclesLate
		);
...
static void GBSIOLockstepNodeWriteSB(struct GBSIODriver* driver, uint8_t value) {
	...
	// === GB SENDS DATA (WRITE) ===
	gb_link_cable_on_write(node->id, value, 8);
...
static uint8_t GBSIOLockstepNodeWriteSC(struct GBSIODriver* driver, uint8_t value) {
	...
	// Begin transfer if bit 7 and 0 are set, AND two nodes attached
	if ((value & 0x81) == 0x81 && attached > 1) {

		// === SESSION START ===
		if (g_link_cable_callbacks.on_session_start) {
			g_link_cable_callbacks.on_session_start(LINK_SYS_GB, node->id);
		}
		...
		if (ATOMIC_CMPXCHG(node->p->masterClaimed, claimed, true)) {
			...
			// GB handshake: swap master/slave
			if (id != 0) {
				unsigned sb;
			...
			mLOG(GB_SIO, DEBUG, "GBSIOLockstepNodeWriteSC: masterClaimed failed\n");
		}
