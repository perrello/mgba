...
#ifdef M_CORE_GBA
static struct GBASIOLockstepCoordinator wasmGbaCoordinator;
static bool wasmLockstepInitialized = false;
#endif
...
#ifdef M_CORE_GBA
static struct GBASIOLockstepCoordinator gbaCoordinator;
#endif
...
#warning This pixel format is unsupported. Please use -DCOLOR_16-BIT -DCOLOR_5_6_5
...
#endif
#ifdef M_CORE_GBA
	GBASIOLockstepCoordinatorInit(&gbaCoordinator);
#endif
	...
	#ifdef M_CORE_GBA
if (!wasmLockstepInitialized) {
    GBASIOLockstepCoordinatorInit(&wasmGbaCoordinator);
    wasmLockstepInitialized = true;
    mLOG(GBA_SIO, INFO, "[LOCKSTEP] Coordinator initialized");
}
#endif
}
...
void retro_deinit(void) {
	...
	#ifdef M_CORE_GBA
GBASIOLockstepCoordinatorDeinit(&wasmGbaCoordinator);
mLOG(GBA_SIO, INFO, "[LOCKSTEP] Coordinator deinitialized");
#endif
}
...
int wasm_link_requested_id(struct mLockstepUser* user) {
    mLOG(GBA_SIO, INFO, "[LOCKSTEP] requested_id()");
    return 0; // tijdelijk
}

void wasm_link_player_changed(struct mLockstepUser* user, int id) {
    mLOG(GBA_SIO, INFO, "[LOCKSTEP] player_changed(%d)", id);
}

void wasm_link_sleep(struct mLockstepUser* user) {
    mLOG(GBA_SIO, INFO, "[LOCKSTEP] sleep()");
}

void wasm_link_wake(struct mLockstepUser* user) {
    mLOG(GBA_SIO, INFO, "[LOCKSTEP] wake()");
}
...
bool retro_load_game(const struct retro_game_info* game) {
    ...
    if (core->platform(core) == mPLATFORM_GBA) {
        ...
        audioSamplesPerFrame =
            (size_t)(((float)targetSampleRate *
                      (float)core->frameCycles(core) /
                      (float)core->frequency(core)) + 0.5f);

        audioBufferSize = ((audioSamplesPerFrame + 1023) / 1024) * 1024;
    ...
    if (core->platform(core) == mPLATFORM_GBA) {
        ...
        // --- Lockstep user callbacks ---
        static struct mLockstepUser user;
        memset(&user, 0, sizeof(user));
        user.requestedId     = wasm_link_requested_id;
        user.playerIdChanged = wasm_link_player_changed;
        user.sleep           = wasm_link_sleep;
        user.wake            = wasm_link_wake;

        // --- Create GBA lockstep driver ---
        static struct GBASIOLockstepDriver driver;
        GBASIOLockstepDriverCreate(&driver, &user);

        // --- Attach to GLOBAL coordinator ---
        GBASIOLockstepCoordinatorAttach(&wasmGbaCoordinator, &driver);

        // --- Replace default SIO driver ---
        struct GBA* gba = core->board;
        GBASIOSetDriver(&gba->sio, &driver.d);

        mLOG(GBA_SIO, INFO, "GBA LOCKSTEP DRIVER ENABLED (WASM)");
    }
#endif
#endif
